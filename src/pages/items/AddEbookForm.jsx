...
import { apiAddEbook } from '../../services/resourses'; // Import the API function
import Sidebar from '../../layouts/Sidebar2'; // Import Sidebar
...
const AddEbookForm = () => {
    ...
    // Handle file change (for ebook file upload)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            await apiAddEbook(formData); // Make the API call
            navigate('/resources'); // Redirect to the resources page upon success
        } catch (err) {
            setError('Failed to post the ebook. Please try again.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="flex">
            ...
            <div className="flex-1 p-8 bg-gray-100">
                <div className="max-w-lg mx-auto p-6 bg-white bg-opacity-40 backdrop-blur-lg rounded-lg shadow-lg">
                    <h2 className="text-2xl text-indigo-900 font-semibold text-center mb-6">Post a New Ebook</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <label htmlFor="file" className="block text-lg text-indigo-900 font-medium">Upload Ebook</label>
                            <input
                                type="file"
                                id="file"
                                className="w-full p-2 text-sm border border-gray-300 rounded-lg bg-transparent focus:outline-none focus:ring-2 focus:ring-blue-400"
